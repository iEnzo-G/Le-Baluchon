...
class ExchangeRateTest: XCTestCase {
    ...
    func test_GivenError_WhenSendRequest() {
        let configuration = URLSessionConfiguration.ephemeral
        configuration.protocolClasses = [URLProtocolStub.self]
        let client = URLSessionHTTPClient(session: .init(configuration: configuration))
        let loader = FixerLoader(client: client)
        ...
        URLProtocolStub.stub(data: data, response: HTTPURLResponse(url: URL(string: "www.a-url.com")!, statusCode: 200, httpVersion: .none, headerFields: .none), error: NSError(domain: "an error", code: 0))
        ...
        loader.load(to: "USD", from: "EUR") { result in
            switch result {
            case let .success(get):
                XCTAssertEqual(get.base, "erreur")
            case let .failure(error):
                XCTFail("Error was not expected: \(error)")
            }
            exp.fulfill()
